...
import { getFeedOrder, setFeedModalState } from '../../services/actions/feed-data';


type Data = {
	children: {ingredients: object[], number: number, createdAt: string, name: string, status: string, _id: string }
}
...
const FeedOrderProducts: FC<Data> = ({children}) => {
  ...
  return ( 
		<Link to={location.pathname ==='/profile/orders' ? {pathname: `/profile/orders/${children._id}`} : {pathname: `/feed/${children._id}`}} onClick={openModal}>
		<div className={`mb-4 pt-6 pl-6 pr-6 pb-6 ${appOrderProductsStyle.wrapper}`}>
				<div className={`mb-6 ${appOrderProductsStyle.numbers}`}>
					<p className={appOrderProductsStyle.number}>{`#${children.number}`}</p>
				...
				<p className={className}>{children.status === 'done' ? 'Выполнен' : children.status === 'pending' ? 'Готовится'
			: 'Создан'}</p>
				}
				...
				<div className={`mt-6 ${appOrderProductsStyle.products}`}>
						{imageFilter?.map((el: any, i: any) => {
							while(i < 6){
								return (
									<div key={i} className={i === 0 ? appOrderProductsStyle.first__child : i === 1 ? appOrderProductsStyle.second__child
									: i === 2 ? appOrderProductsStyle.third__child : i === 3 ? appOrderProductsStyle.fourth__child : 
									i === 4 ? appOrderProductsStyle.fifth__child : i === 5 && imageFilter.length > 6 ? appOrderProductsStyle.sixth__child : 
									appOrderProductsStyle.last__child }>
										...
										<p className={appOrderProductsStyle.qty}>
												...
												<img src={el} alt='ingredient image' className={appOrderProductsStyle.size}/>
											</div>
										</div>
									</div>
								)
							...
							<div className={appOrderProductsStyle.full__price}>
								<p className={`pr-2 ${appOrderProductsStyle.price}`}>{priceFilter}</p>
